...
% system using PCET featuring a shorthand method to create the PCET input
...
% define LTI system
x = sym('[x1;x2;x3]');
A = sym('[-R1,0,0; R1,-R2,0; 0,R2,-R3]'); % use commas!
B = sym([1,0;0,1;0,0]);
C = sym([0,0,1]);
D = sym([0,0]);
u = sym('[u1;u2]');
...
% create input structs for PCET from LTI system
...
PCETwriteFiles(sys,'ex2_ODE.m','ex2_OUT.m','ex2_MCODE.m','ex2_MCOUT.m')

%% (2.a) simulate system and compute moments
% run PCE simulation
results = PCETsimGalerkin(sys,'ex2_ODE','ex2_OUT',simoptions);

% compute moments from simulation results and store also in results
results = PCETcalcMoments(sys,MomMats,results);

% run Monte-Carlo simulations
samples = PCETsample(sys,'variables',mc_samples);
mcresults = PCETsimMonteCarlo(sys,'ex2_MCODE','ex2_MCOUT',samples,simoptions,'method','moments');
...
mccomplete = PCETsimMonteCarlo(sys,'ex2_MCODE','ex2_MCOUT',mc_samples,simoptions);
