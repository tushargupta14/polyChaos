...
% mySystem = PCETcompose(states,parameters,inputs,outputs,order);
sys = PCETcompose(states,parameters,[],[],pce_order);
PCETwriteFiles(sys,'ex3_RHS',[],'ex3_MCRHS');

%% (2) simulate system
% run galerkin-PCE simulation
results = PCETsimGalerkin(sys,'ex3_RHS',[],simoptions);

% compute moments from simulation results
sys.MomMats = PCETmomentMatrices(sys,4);
results.A.moments = PCETcalcMoments(sys,sys.MomMats,results.A.pcvals);
results.B.moments = PCETcalcMoments(sys,sys.MomMats,results.B.pcvals);

% run Monte-Carlo simulations
mcresults = PCETsimMonteCarlo(sys,'ex3_MCRHS',[],mc_samples,simoptions,'method','moments');
...
sys = PCETupdate(sys,'A',[3 5],'p_2',[0.7 0.9]);

% run Galerkin-PCE simulation
results2 = PCETsimGalerkin(sys,'ex3_RHS',simoptions);

% compute moments from simulation results (alternative function call)
results2 = PCETcalcMoments(sys,sys.MomMats,results2);

% run Monte-Carlo simulations
mcresults2 = PCETsimMonteCarlo(sys,'ex3_MCRHS',[],mc_samples,simoptions,'method','moments');
